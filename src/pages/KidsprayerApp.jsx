// src/pages/KidsPrayerApp.jsx
// ============================================================
// تعليم الوضوء والصلاة خطوة بخطوة بالصور — نفس نمط باقي الألعاب:
// onComplete/onExit props. onComplete بينادى مرتين محتملتين:
// مرة لما الطفل يخلّص الوضوء، ومرة لما يخلّص الصلاة (مكافأة
// مختلفة لكل واحدة، متحكم فيها من KidsDashboard).
// ============================================================
import { useState } from "react";
import styles from "./KidsPrayerApp.module.css";
...
export default function KidsPrayerApp({ onComplete, onExit } = {}) {
  ...
  return (
    <div className={styles.appFrame}>
      {onExit && (
        <button className={styles.exitBtn} onClick={onExit}>→ رجوع</button>
      )}

      {screen === "menu" && (
        <>
          <h1 className={styles.appMainTitle}>الصلاة والوضوء</h1>
          <p className={styles.appSubTitle}>سوف تتعلم كيف تتوضأ وتصلي خطوة بخطوة</p>

          <button className={`${styles.orangeButton} ${styles.menuBtn}`} onClick={() => { setScreen("wudu"); setCurrentStep(0); }}>
            <span>ابدأ الوضوء (8 خطوات)</span>
            <span style={{ fontSize: "20px" }}>🚰</span>
          </button>

          <button className={`${styles.orangeButton} ${styles.menuBtn}`} onClick={() => { setScreen("prayer"); setCurrentStep(0); }}>
            <span>ابدأ الصلاة (6 خطوات)</span>
            <span style={{ fontSize: "20px" }}>⭐</span>
          </button>

          <div className={styles.starsRow}>
            <span>⭐</span><span>⭐</span><span>⭐</span><span>⭐</span>
            <span className={styles.starsText}>نقاط عند الإكمال</span>
          </div>
        </>
      )}
      ...
      {screen === "wudu" && (
        <>
          <h1 className={styles.stepTitle}>{wuduSteps[currentStep].title}</h1>
          <p className={styles.stepInstruction}>{wuduSteps[currentStep].instruction}</p>
          ...
          <div className={styles.badgeCounter}>{wuduSteps[currentStep].id}</div>
          <button className={`${styles.orangeButton} ${styles.actionBtn}`} onClick={nextWuduStep}>
      ...
      {screen === "wuduComplete" && (
        <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <h1 className={styles.congratsTitle}>عاش يا بطل 🎉</h1>
          <p className={styles.congratsSub}>ما شاء الله، انت الآن طاهر. هل تتعلم الصلاة؟</p>
          <button className={`${styles.orangeButton} ${styles.menuBtn}`} onClick={() => { setScreen("prayer"); setCurrentStep(0); }}>نعم، تعلم الصلاة</button>
          <button className={`${styles.orangeButton} ${styles.menuBtn} ${styles.secondaryStyle}`} onClick={resetToMenu}>الرجوع للقائمة</button>
        </div>
      )}

      {screen === "prayer" && (
        <>
          <h1 className={styles.stepTitle}>{prayerSteps[currentStep].title}</h1>
          <p className={styles.stepInstruction}>{prayerSteps[currentStep].instruction}</p>
          ...
          <div className={styles.badgeCounter}>{prayerSteps[currentStep].id}</div>
          <button className={`${styles.orangeButton} ${styles.actionBtn}`} onClick={nextPrayerStep}>
      ...
      {screen === "prayerComplete" && (
        <div style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center" }}>
          <h1 className={styles.congratsTitle}>عاش يا بطل 🏆</h1>
          <p className={styles.congratsSub}>تعلمت الوضوء والصلاة كاملاً، ما شاء الله</p>
          <button className={`${styles.orangeButton} ${styles.menuBtn}`} onClick={resetToMenu}>الرجوع للقائمة</button>
          {onExit && (
            <button className={`${styles.orangeButton} ${styles.menuBtn} ${styles.secondaryStyle}`} onClick={onExit}>رجوع للألعاب</button>
          )}
        </div>
      )}
    </div>
